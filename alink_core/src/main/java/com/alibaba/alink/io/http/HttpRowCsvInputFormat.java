...
public class HttpRowCsvInputFormat implements InputFormat <Row, HttpFileInputSplit> {
    ...
    public HttpRowCsvInputFormat(String path, TypeInformation <?>[] fieldTypes,
    ...
    public Row nextRecord(Row record) throws IOException {
        if (readLine()) {
            //            System.out.println(String.format("SplitNo = %d, The %d-th record, currBufLen=%d,
            // currOffset=%d, currLen=%d, limit=%d, readPos=%d, splitLength=%d",
            //                    split.splitNo, (debug_cnt++), currBuffer.length, currOffset, currLen, limit,
            // readPos, splitLength));

            //            debug_cnt++;
            //            if(debug_cnt % 100000 == 0)
            //                System.out.println("split " + split.splitNo + " reading record " + debug_cnt);
    ...
    private boolean fillBuffer(int offset) throws IOException {
        ...
        if (tryTimes >= 10) { throw new RuntimeException("Fail to read from http connection after trying 10 times."); }
    ...
    public HttpFileInputSplit[] createInputSplits(int minNumSplits) throws IOException {
        ...
        if (contentType.toLowerCase().indexOf("utf-8") < 0) {
            throw new RuntimeException("Data charset is not utf-8: " + contentType);
        }
        ...
        if (!splitable) { throw new RuntimeException("The http server does not support range reading."); }
    ...
    protected Row readRecord(Row reuse, byte[] bytes, int offset, int numBytes) throws IOException {
        Row reuseRow;
        if (reuse == null) { reuseRow = new Row(fieldTypes.length); } else { reuseRow = reuse; }
