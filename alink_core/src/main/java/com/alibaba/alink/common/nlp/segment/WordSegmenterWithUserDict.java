...
public class WordSegmenterWithUserDict implements Serializable {
    private WordDictWithUserDefined wordDict;
    ...
    public WordSegmenterWithUserDict() {
        wordDict = new WordDictWithUserDefined();

    }

    public void addUserWord2Dict(String word) {

        WordDictionary dict = wordDict.getDictionary();
    ...
    private Map <Integer, List <Integer>> createDAG(String sentence) {
        ...
        DictNode trie = wordDict.getDictionary().getTrie();
    ...
    private Map <Integer, Pair <Integer>> calc(String sentence, Map <Integer, List <Integer>> dag) {
        ...
        for (int i = N - 1; i > -1; i--) {
            ...
            for (Integer x : dag.get(i)) {
                double freq = wordDict.getDictionary().getFreq(sentence.substring(i, x + 1)) + route.get(x + 1).freq;
                if (null == candidate) {
    ...
    public List <SegToken> process(String paragraph, SegMode mode) {
        ...
        for (int i = 0; i < paragraph.length(); ++i) {
            ...
            if (CharacterUtil.ccFind(ch)) { sb.append(ch); } else {
                if (sb.length() > 0) {
                    // process
                        ...
                        for (String token : sentenceProcess(sb.toString())) {
                            if (token.length() > 2) {
                                ...
                                for (; j < token.length() - 1; ++j) {
                                    ...
                                    if (wordDict.getDictionary().containsWord(gram2)) {
                            ...
                            if (token.length() > 3) {
                                ...
                                for (; j < token.length() - 2; ++j) {
                                    ...
                                    if (wordDict.getDictionary().containsWord(gram3)) {
                ...
                if (wordDict.getDictionary().containsWord(paragraph.substring(i, i + 1))) {
                    tokens.add(new SegToken(paragraph.substring(i, i + 1), offset, ++offset));
                } else { tokens.add(new SegToken(paragraph.substring(i, i + 1), offset, ++offset)); }
            }
        }
        if (sb.length() > 0) {
            if (mode == SegMode.SEARCH) {
            ...
            if (y - x == 1) { sb.append(lWord); } else {
                if (sb.length() > 0) {
                    ...
                    if (buf.length() == 1) {
                        ...
                        if (wordDict.getDictionary().containsWord(buf)) {
                            tokens.add(buf);
                        } else {
