...
public class DictNode implements Comparable <DictNode> {
    ...
    // 数组方式存储结构
    private DictNode[] childrenArray;
    ...
    // 当前DictNode状态 ,默认 0 , 1表示从根节点到当前节点的路径表示一个词
    ...
    Hit match(char[] charArray, int begin, int length, Hit searchHit) {
        ...
        DictNode[] segmentArray = this.childrenArray;
        Map <Character, DictNode> segmentMap = this.childrenMap;

        // STEP1 在节点中查找keyChar对应的DictNode
        if (segmentArray != null) {
            // 在数组中查找
            DictNode keySegment = new DictNode(keyChar);
            ...
            // 在map中查找
            ds = (DictNode)segmentMap.get(keyChar);
        }

        // STEP2 找到DictNode，判断词的匹配状态，是否继续递归，还是返回结果
        if (ds != null) {
            if (length > 1) {
                // 词未匹配完，继续往下搜索
                ...
                if (ds.hasNextNode()) {
                    ...
                    searchHit.setPrefix();
                    // 记录当前位置的DictNode
                    searchHit.setMatchedDictNode(ds);
                }
                return searchHit;
            }

        }
        // STEP3 没有找到DictNode， 将HIT设置为不匹配
        return searchHit;
    }
    ...
    private DictNode lookforSegment(Character keyChar, int create) {
        ...
        if (this.storeSize <= ARRAY_LENGTH_LIMIT) {
            // 获取数组容器，如果数组未创建则创建数组
            DictNode[] segmentArray = getChildrenArray();
            // 搜寻数组
            DictNode keySegment = new DictNode(keyChar);
            int position = Arrays.binarySearch(segmentArray, 0, this.storeSize, keySegment);
            if (position >= 0) {
     ...
     * 获取数组容器 线程同步方法
     */
    private DictNode[] getChildrenArray() {
        if (this.childrenArray == null) {
            synchronized (this) {
                if (this.childrenArray == null) {
                    this.childrenArray = new DictNode[DictNode.ARRAY_LENGTH_LIMIT];
                }
            }
        }
    ...
    private Map <Character, DictNode> getChildrenMap() {
        if (this.childrenMap == null) {
            synchronized (this) {
                if (this.childrenMap == null) {
                    this.childrenMap = new HashMap <Character, DictNode>(ARRAY_LENGTH_LIMIT * 2, 0.8f);
                }
            }
        }
     ...
     * @param segmentArray
     */
    private void migrate(DictNode[] segmentArray, Map <Character, DictNode> segmentMap) {
        for (DictNode segment : segmentArray) {
            if (segment != null) {
