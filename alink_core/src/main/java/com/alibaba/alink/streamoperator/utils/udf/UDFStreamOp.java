 ...
 */
public class UDFStreamOp extends StreamOperator {
    ...
    public UDFStreamOp(String selectedColName, String outputColName, UserDefinedFunction udf, String[] keepColNames) {
        this(selectedColName, new String[]{outputColName}, udf, keepColNames);
    }

    public UDFStreamOp(String selectedColName, String[] outputColNames, UserDefinedFunction udf, String[] keepColNames) {
        super(null);
    ...
    public StreamOperator linkFrom(StreamOperator in) {
        ...
        if (TableUtil.findIndexFromName(outputColNames, this.selectedColName) < 0) {
            ...
            if (this.udf instanceof TableFunction) {
                this.table = UDTFStreamOp.exec(in, selectedColName, outputColNames, (TableFunction <Row>) udf, this.keepColNames);
            } else if (this.udf instanceof ScalarFunction) {
                this.table = UDSFStreamOp.exec(in, selectedColName, outputColNames[0], (ScalarFunction) udf, this.keepColNames);
            } else {
            ...
            if (this.udf instanceof TableFunction) {
                this.table = UDTFStreamOp.exec(in, selectedColName, outputColNames, (TableFunction <Row>) udf, this.keepColNames).as(clauseAS);
            } else if (this.udf instanceof ScalarFunction) {
                this.table = UDSFStreamOp.exec(in, selectedColName, outputColNames[0], (ScalarFunction) udf, this.keepColNames).as(clauseAS);
            } else {
                throw new RuntimeException("Not supported yet!");
            }
        }
